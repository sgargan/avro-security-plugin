...
/**
 * <code>TicketCache</code> is used to store currently valid {@link Authentication}s for Users so that the
 * overhead of Authentication and Authorization can be avoided across multiple invocations.
 */
public class TicketCache<AuthType> {
	...
	private PriorityBlockingQueue<Ticket> expiryQueue = new PriorityBlockingQueue<Ticket>(100, new TicketComparator());
	...
	/**
	 * Create a ticket and cache the given credentials and an arbitrary authentication object for retrieval
	 * with the ticket.
	 * 
	 * @param username the principals username.
	 * @param password the principals password
	 * @param auth the authentication object that should be cached for the ticket
	 * 
	...
	/**
	 * Cancel a ticket, removing it and its associated authentication data from the cache.
	 * 
	...
	/**
	 * Remove any ticket currently held for a username and thus force them to re-authenticate.
	 * 
	...
	private Ticket buildTicket(CharSequence username, CharSequence password, long expiry) {
		...
		ticket.digest = ByteBuffer.wrap(digest.digest((username + ":" + password + expiry).getBytes()));
	...
	/**
	 * Allow injection of a cache map. For use in distributed caching of tickets.
	 * 
	 * @param ticketCache
	 */
			...
			while (true) {
				...
				try {
					...
					if (log.isDebugEnabled()) {
						...
						if (auth != null) {
							log.debug("Authentication ticket for user '{}' has expired and will be removed",
									auth.username);
						}
						if (auth != null) {
							ticketsByUser.remove(auth.username);
